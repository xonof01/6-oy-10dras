      ...
      <div className='m-3'>
        ...
        <div className='flex items-center gap-10'>
          <ul className='w-[400px] border-[3px] border-slate-700 ml-[20px] h-[90vh] rounded-lg bg-slate-300 p-3 gap-5 flex flex-col overflow-y-auto shadow-lg'>
              {users.map(item => (
                  <li key={item.id} className='bg-white mx-auto bg-antiquewhite w-[370px] p-4 rounded-md shadow-lg hover:scale-[1.05] duration-300'> 
          ...
          <ul className='w-[400px] border-[3px] border-slate-700 ml-[20px] h-[90vh] rounded-lg bg-slate-300 p-4 gap-5 flex flex-col overflow-y-auto shadow-lg'>
              {posts.map(item => (
                  <li key={item.id} className='bg-white mx-auto bg-antiquewhite w-[370px] p-3 rounded-md shadow-lg hover:scale-[1.05] duration-300'> 
                    <div className='p-1 flex flex-col gap-3 '>
                        ...
                        <button onClick={()=> setPostId(item.id)} className='w-full font-semibold bg-green-600 text-white py-[10px] block mt-2 hover:opacity-[70%] rounded-md'>SHow Comments</button>
                    </div>
                </li>
              ))}
          </ul>
          <ul className='w-[400px] border-[3px] border-slate-700 ml-[20px] h-[90vh] rounded-lg bg-slate-300 p-3 gap-5 flex flex-col overflow-y-auto shadow-lg'>
              {comments.map(item => (
                  <li key={item.id} className='bg-white mx-auto bg-antiquewhite w-[370px] p-3 rounded-md shadow-lg hover:scale-[1.05] duration-200'> 
                    <div className='p-1 flex flex-col gap-3'>
                        <strong><strong>Id: </strong>{item.id}</strong>
